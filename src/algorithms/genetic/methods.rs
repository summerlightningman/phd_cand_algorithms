...
use super::types::{MutateFunc, SelectFunc, CrossoverFunc};
...
impl Crossover {
    pub fn one_point<T: Clone>(point_idx: Option<usize>) -> CrossoverFunc<T> {
         CrossoverFunc(Box::new(move |a: &Individual<T>, b: &Individual<T>, rng: &mut ThreadRng| {
...
impl Select {
    ...
    pub fn stochastic<T: Clone>(rate: Option<f32>) -> SelectFunc<T> {
        SelectFunc(Box::new(move |population: Population<T>, _: &Purpose, rng: &mut ThreadRng| {
            let count = helpers::get_count_by_rate::<T>(population.len(), rate.unwrap_or(RATE_DEFAULT));
    ...
    pub fn best_n<T: Clone>(rate: Option<f32>)-> SelectFunc<T> {
        SelectFunc(Box::new(move |mut population: Population<T>, purpose: &Purpose, _: &mut ThreadRng| {
            let count = helpers::get_count_by_rate::<T>(population.len(), rate.unwrap_or(RATE_DEFAULT));
